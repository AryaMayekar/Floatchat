...
def create_folder_structure(base_dir, year, month):
    ...
    folder_path = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", base_dir, f"argo_{year}", month_name)
    os.makedirs(folder_path, exist_ok=True)
    return folder_path

def organize_existing_netcdf_files():
    """Move existing files from NETcdf_Data to organized structure"""
    old_dir = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", "NETcdf_Data")
...
def organize_existing_excel_files():
    ...
    base_excel_folder = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", "rough_csv_Data")
...
def calculate_surface_temp(temp_data, pres_data):
    ...
    try:
        surface_mask = (pres_data >= 0) & (pres_data <= 20) & np.isfinite(temp_data) & np.isfinite(pres_data)
...
def convert_netcdf_to_csv():
    ...
    # Base folders
    base_nc_folder = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", "NETcdf_Data", "argo_" + str(YEAR))
    base_excel_folder = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", "rough_csv_Data")
...
def organize_existing_final_excel_files():
    ...
    base_final_folder = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", "final_csv")
...
def find_csv_files_in_structure(base_folder):
    ...
    # Update base folder path to include Data directory and Non-BCG floats
    full_base_folder = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", base_folder)
...
def create_final_csv_summaries():
    ...
    final_excel_folder = os.path.join("Data preprocessing and testing files", "Data", "Non-BCG floats", "final_csv")
    ...
    for (year, month_name), month_files in files_by_month.items():
        ...
        # Add FLOAT_SERIAL_NO if available
        if 'FLOAT_SERIAL_NO' in df.columns:
            # Insert FLOAT_SERIAL_NO after PLATFORM_NUMBER
            groupby_cols.insert(1, 'FLOAT_SERIAL_NO')
            print("    Including FLOAT_SERIAL_NO in profile summary grouping")
        ...
        if missing_cols:
            ...
            if 'PLATFORM_NUMBER' not in groupby_cols:
                base_cols = ['PLATFORM_NUMBER', 'CYCLE_NUMBER', 'DATE']
                if 'FLOAT_SERIAL_NO' in df.columns:
                    base_cols.insert(1, 'FLOAT_SERIAL_NO')
        ...
        for name, group in grouped:
            # Handle different grouping scenarios based on grouping columns
            summary = {'row_count': len(group)}
            
            # Extract values from group name tuple based on grouping columns order
            if len(groupby_cols) == 1:
                # Single grouping column
                summary[groupby_cols[0]] = name
            else:
                # Multiple grouping columns - map them in order
                for i, col in enumerate(groupby_cols):
                    summary[col] = name[i]
            
            # Ensure essential columns exist with fallback values
            if 'PLATFORM_NUMBER' not in summary:
                summary['PLATFORM_NUMBER'] = group['PLATFORM_NUMBER'].iloc[0] if 'PLATFORM_NUMBER' in group.columns else 'Unknown'
            
            if 'FLOAT_SERIAL_NO' not in summary and 'FLOAT_SERIAL_NO' in group.columns:
                summary['FLOAT_SERIAL_NO'] = group['FLOAT_SERIAL_NO'].iloc[0]
            
            if 'n_levels' not in summary:
                summary['n_levels'] = group['n_levels'].iloc[0] if 'n_levels' in group.columns else len(group)
            
            if 'DATE' not in summary:
                summary['DATE'] = group['DATE'].iloc[0] if 'DATE' in group.columns else pd.to_datetime('2025-01-01').date()
            ...
            if 'TEMP' in group.columns and 'PRES' in group.columns:
                try:
                    ...
                    if len(temp_data) > 0 and len(pres_data) > 0:
                        ...
                        # Calculate surface temperature (0-20 dbar)
                        surface_temp_avg = calculate_surface_temp(temp_data, pres_data)
                        summary['surface_temp_avg_C'] = surface_temp_avg
            ...
            for col in df.columns:
                if col in ['PLATFORM_NUMBER', 'n_levels', 'DATE', 'row_count', 'JULD', 'JULD_LOCATION', 'PRES_ADJUSTED']:
                    continue  # Skip JULD, JULD_LOCATION, and PRES_ADJUSTED since PRES_ADJUSTED is empty
                ...
                try:
                    ...
                    if df[col].dtype in ['object', 'string']:
                        ...
                        if len(numeric_data) > 0:
                            if col in ['PRES']:
                                # For PRES column, calculate mean pressure and store directly in PRES column
                                summary[f'{col}'] = float(numeric_data.mean())
...
def main():
    ...
    try:
        ...
        print("="*60)
        print("Results:")
        print(f"• NetCDF files: Data/Non-BCG floats/NETcdf_Data/argo_{YEAR}/[month]/")
        print(f"• Individual CSV files: Data/Non-BCG floats/rough_csv_Data/{YEAR}/[month]/")
        print(f"• Final summaries: Data/Non-BCG floats/final_csv/{YEAR}/[month]/final_[MON]_days.csv")
